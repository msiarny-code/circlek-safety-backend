...
def generate_support_staff_pdf(data):
    ...
    # Title
    title = Paragraph("Store Safety Walk - BU Support Staff", title_style)
    story.append(title)
    
    subtitle1 = Paragraph("We want all employees and customers to go home in the same condition they arrived!", subtitle_style)
    story.append(subtitle1)
    
    subtitle2 = Paragraph("Your help is needed to ensure that we practice Safety First everyday in our stores", subtitle_style)
    ...
    header_data = [
        ...
        ['Title:', data.get('title', '') or data.get('employeeRole', ''), 'Date/Time:', f"{data.get('date', '')} {data.get('time', '')} UTC-5" if data.get('time') else data.get('date', '')]
    ]
    ...
    # Generate filename
    filename = f"CircleK_Store{data.get('storeNumber', '')}_SupportStaff_{data.get('date', '')}.pdf"
...
if __name__ == '__main__':
    ...
    # Generate PDF
    pdf_bytes, filename = generate_support_staff_pdf(data)
